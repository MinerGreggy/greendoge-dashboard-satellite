...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'greendoge-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('greendoge').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('greendoge')[service]);
